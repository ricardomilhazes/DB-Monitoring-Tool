...
#hist_resourse = cur.execute("select * from dba_hist_resource_limit")

#user_privileges = cur.execute("select  grantee, privilege from dba_sys_privs where grantee = 'GREY' order by grantee desc")

#con= cx_Oracle.connect("dbmonitoring", "pass", "172.16.201.131/orcl", mode = cx_Oracle.SYSDBA, encoding="UTF-8")
...
for k,v in datafiles_dict.items():
	sql = "merge into dbmonitoring.datafile dtf using (select tablespace_id from dbmonitoring.tablespace where name = :tablespace_name) src on (dtf.tablespace_id = src.tablespace_id and dtf.name = :file_name) when matched then update set used_mb = :used_mb, free_mb = :free_mb, total_mb = :total_mb, free_percentage = :free_percentage, max_size = :max_size, autoextensible = :autoextensible, status = :status, online_status = :online_status, timestamp = (select current_timestamp from dual) when not matched then insert (datafile_id, tablespace_id, name, used_mb, free_mb, total_mb, free_percentage, max_size, autoextensible, status, online_status, timestamp) values (datafile_sq.nextval,(select tablespace_id from dbmonitoring.tablespace where name = :tablespace_nam),:file_name, :used_mb, :free_mb, :total_mb, :free_percentage, :max_size, :autoextensible, :status, :online_status, (select current_timestamp from dual))"
...
for k,u in users_dict.items():
	sql = "merge into dbmonitoring.users u using (select (:username) as name from dual) src on (u.username = src.name) when matched then update set account_status = :account_stats, expiration_date = :expiration_date, default_tablespace = (select tablespace_id from dbmonitoring.tablespace where name = :default_tablespace), temp_tablespace = (select tablespace_id from dbmonitoring.tablespace where name = :temp_tablespace), creation_date = :creation_date, timestamp = (select current_timestamp from dual) when not matched then insert (user_id, username, account_status, expiration_date, default_tablespace, temp_tablespace, creation_date, timestamp) values (user_sq.nextval, :username, :account_status, :expiration_date, (select tablespace_id from dbmonitoring.tablespace where name = :default_tablespace), (select tablespace_id from dbmonitoring.tablespace where name = :temp_tablespace), :creation_date, (select current_timestamp from dual))"
...
for k,v in sga_dict.items():
	sql = "merge into dbmonitoring.sga s using (select :name as name from dual) src on (s.name = src.name) when matched then update set s.used = :value when not matched then insert (sga_id, name, used, timestamp) values (sga_sq.nextval, :name, :value, (select current_timestamp from dual))"

	values = (s.name, s.value, s.name, s.value)
...
pga = cur.execute("select name, value from v$pgastat")
...
for k,v in pga_dict.items():
	sql = "merge into dbmonitoring.pga p using (select :name as name from dual) src on (p.name = src.name) when matched then update set p.used = :value, p.unit = :unit when not matched then insert (pga_id, name, used, unit, timestamp) values (pga_sq.nextval, :name, :value, :unit, (select current_timestamp from dual))"
...
cpu = cur.execute("select username, sum(cpu_usage) from (select se.username, round (value/100) as cpu_usage from v$session se, v$sesstat ss, v$statname st where (ss.statistic# = st.statistic# and name like '%cpu used by this session%' and se.sid = ss.sid and se.username is not null)order by value desc)group by username")
...
for k,v in cpu_dict.items():
	sql = "merge into dbmonitoring.cpu c using (select :username as username from dual) src on (c.username = src.username) when matched then update set c.cpu_count = :count, c.timestamp = (select current_timestamp from dual) when not matched then insert (cpu_instance_id, username, cpu_count, timestamp) values (cpu_sq, :username, :cpu_count, (select current_timestamp from dual))"
